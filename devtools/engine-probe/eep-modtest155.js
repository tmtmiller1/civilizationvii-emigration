// eep-modtest155.js - mod test 154 again, but the host is first given 2000 Gold so the Gold-costing stance is
...
async function run() {
  ...
  if (!host || !foreignSig) { emit("no host or foreign major: host=" + !!host + " foreign=" + !!foreignSig); emit("DONE modtest155 finished"); return; }
  ...
  if (!dlg) { emit("no enclave dialog appeared; dialogs=" + document.querySelectorAll("screen-dialog-box").length); emit("DONE modtest155 finished"); return; }
  await later(1500);
  ...
  await later(3000);
  emit("DONE modtest155 finished");
}

emit("modtest155 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest155 finished"); }); }, 12000); return; }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest155 finished"); }
}
