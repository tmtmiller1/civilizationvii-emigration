// eep-modtest112.js - What combat evidence does the engine actually hand us, and does it reach into fog?
//
...
import { combatEvidence } from "/emigration/ui/emigration-combat-events.js";
...
function reportEvidence() {
  ...
  for (const p of safe(() => Players.getAlive(), []) || []) {
    ...
    for (const c of cities) {
      ...
      const ev = safe(() => combatEvidence(key), null);
...
function finish() {
  ...
  reportEvidence();
  emit("TALLY " + J(counts));
  setTimeout(() => emit("DONE modtest112 finished"), 4000);
}

function run() {
  local = safe(() => GameContext.localPlayerID, -1);
  emit("modtest112 run local=" + local);
...
emit("modtest112 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") {
    setTimeout(() => { try { run(); } catch (e) { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest112 finished"); } }, 12000);
    return;
  }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest112 finished"); }
}
