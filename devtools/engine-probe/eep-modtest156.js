// eep-modtest156.js - the stance figures moved onto the buttons (user request): mod test 155 again, reading each
...
async function run() {
  ...
  emit("GRANT +2000 Gold so the priced stance is affordable");
  safe(() => Players.grantYield(local, YieldTypes.YIELD_GOLD, 2000));
  await later(3000);
  emit("POOLS after grant " + J(pools(local)));
  // 1. VIEW
  ...
  if (!dlg) { emit("no enclave dialog appeared; dialogs=" + document.querySelectorAll("screen-dialog-box").length); emit("DONE modtest156 finished"); return; }
  await later(1500);
  ...
  emit("DIALOG icons=" + icons + " buttons=" + J(buttons.map((b) => ({ caption: b.getAttribute("caption"), disabled: b.getAttribute("disabled"),
    iconsInLabel: b.querySelectorAll("fxs-font-icon, .fxs-font-icon, [class*=icon]").length, rawTagVisible: /\[icon:/.test(b.textContent || "") }))));
  ...
  const btn = buttons.find((b) => String(b.getAttribute("caption") || "").startsWith(first.label.split("  ")[0])) || buttons[0];
  ...
  await later(3000);
  emit("DONE modtest156 finished");
}

emit("modtest156 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest156 finished"); }); }, 12000); return; }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest156 finished"); }
}
