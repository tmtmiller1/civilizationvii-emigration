// eep-modtest60.js - the placement prompt, across a REAL turn. Mod test 59 showed the raw write
...
async function run() {
  ...
  emit("DONE modtest60 finished");
}

emit("modtest60 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest60 finished"); }); }, 10000); return; }
  beginTries++;
