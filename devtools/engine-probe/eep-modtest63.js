// eep-modtest63.js - mod test 61 with the screenshots fixed: a real pre-write shot, a closer camera, and any
// screen that opens at turn start (mod test 61's turn-67 shots showed an AI peace-deal proposal over the map) closed
// before each shot. Header of mod test 61 follows.
// eep-modtest63.js - mod test 60 plus SCREENSHOTS (runner SHOT lines, captured by game window id): London and
...
async function closeScreens() {
  // Turn-start screens: a confirmation takes OK, a proposal takes Reject, anything else its close button (mod test 62
  // pressed the peace deal's X six times and only raised "Close peace deal proposal?" each time).
  ...
  for (let i = 0; i < 10; i++) {
    ...
    emit("CLOSED " + JSON.stringify(caption(pick) || pick.tagName));
    await later(2500);
    if (!document.querySelector("fxs-close-button") && !buttons.some((b) => /^(ok|reject)$/i.test(caption(b)))) return;
  }
}
...
async function shot(label, a, b) {
  for (const [tag, c] of [[label, a], [label + "-control", b]]) {
    if (!c) continue;
    await closeScreens();
    safe(() => Camera.lookAtPlot(c.location, { zoom: 0.25 }));
    await later(5000);
...
async function run() {
  ...
  await shot("prewrite", city, control[0]);
  for (const c of targets) safe(() => c.addRuralPopulation(1));
  await later(3000);
  await shot("postwrite", city, control[0]);
  ...
  await later(10000);
  await shot("afterturn-london", city, null);
  await shot("afterturn-philadelphia", control[0] || city, null);
  ...
  emit("DONE modtest63 finished");
}

emit("modtest63 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest63 finished"); }); }, 10000); return; }
  beginTries++;
