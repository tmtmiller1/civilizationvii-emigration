// eep-modtest98.js - WATCH city.isInfected go true during a plague crisis.
...
function finish(why) {
  ...
  emit("DONE modtest98 finished");
}

function run() {
  local = safe(() => GameContext.localPlayerID, -1);
  emit("modtest98 run local=" + local);
  dbReport();
  snapshot("START");
...
emit("modtest98 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") {
    setTimeout(() => { try { run(); } catch (e) { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest98 finished"); } }, 10000);
    return;
  }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest98 finished"); }
}
