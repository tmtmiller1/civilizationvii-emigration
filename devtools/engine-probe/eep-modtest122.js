// eep-modtest122.js - Screenshot the real Options screen: Mods tab, conflict sliders, collapsible advanced sections.
//
...
function step(i) {
  ...
  if (!api || typeof api.options !== "function") { emit("NO options() on globalThis.emigration"); emit("DONE modtest122 finished"); return; }
  if (i >= TABS.length) { setTimeout(() => emit("DONE modtest122 finished"), 3000); return; }
  const tab = TABS[i];
  emit("OPEN tab " + tab);
...
function poll() {
  ...
  if (tries < 90) setTimeout(poll, 2000); else { emit("LOAD gave up"); emit("DONE modtest122 finished"); }
}
emit("modtest122 attached");
