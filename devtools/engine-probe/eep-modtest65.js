// eep-modtest65.js - mod test 64 plus the attribution step: after turn 67, place Philadelphia's natural point and
...
async function run() {
  ...
  emit("ENDTURN with only mod-written points pending: advanced=" + moved + " turn=" + safe(() => Game.turn) + " blocker=" + J(blockerInfo()) + " NEW_POPULATION=" + J(popNow()));
  emit("VERDICT promptForModPoints=" + (onlyWritten.length > 0) + " blocksTurn=" + onlyWritten.some((n) => n.blocks) +
    " naturalPlaced=" + J(control.map((c) => snap(c).pending)) + " writtenStillPending=" + J(targets.map((c) => snap(c).pending)));
  emit("DONE modtest65 finished");
}

emit("modtest65 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest65 finished"); }); }, 10000); return; }
  beginTries++;
