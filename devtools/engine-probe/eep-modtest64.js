// eep-modtest64.js - mod test 60 with notification types read the base game's way (getType, then getTypeName),
...
async function run() {
  ...
  emit("VERDICT advanced=" + advanced + " newPopulationNotes=" + pop.length + " byTarget=" + J(pop.map((n) => n.target)) +
    " writtenStillReady=" + J(targets.map((c) => snap(c).ready)) + " controlGrew=" + J(control.map((c) => snap(c).ready)));
  emit("DONE modtest64 finished");
}

emit("modtest64 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest64 finished"); }); }, 10000); return; }
  beginTries++;
