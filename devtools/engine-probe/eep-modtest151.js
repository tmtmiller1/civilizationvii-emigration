// eep-modtest151.js - rerun of mod test 149 to watch item 2 in the hover panel (149 read it after the lens had been
// switched off, so no panel drew). Otherwise identical. Original header:
// the ethnicity-audit fixes, watched together in game (docs/ethnicity-audit-solution-designs.md).
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(() => { afterTurn().catch((e) => { emit("afterTurn threw " + e + " " + (e && e.stack)); emit("DONE modtest151 finished"); }); }, 8000);
});
...
async function finish() {
  ...
  if (pass) {
    ...
    if (tile) {
      ...
      await later(6000);
      emit("ITEM2 lens=" + safe(() => LensManager.activeLens) + " tile shares=" + J(tile.shares.map((x) => x.civ + ":" + (x.share * 100).toFixed(1))));
  ...
  emit("DONE modtest151 finished");
}

async function run() {
  local = safe(() => GameContext.localPlayerID, -1);
  emit("modtest151 run local=" + local + " turn=" + safe(() => Game.turn) + " emigrationBooted=" + (typeof globalThis.emigration)
    + " version=" + safe(() => compositionVersion()));
  await later(6000);
  await dismissPopups();
  if (!setup()) { emit("DONE modtest151 finished"); return; }
...
emit("modtest151 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") {
    setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest151 finished"); }); }, 10000);
    return;
  }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest151 finished"); }
}
