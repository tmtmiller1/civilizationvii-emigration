// eep-modtest104.js - lenses and the banner bar on 1.5.0, tested correctly at last.
//
...
function lensChecks() {
  ...
  for (const [label, id] of [["prosperity", PROS], ["ethnicity", ETHN]]) {
    safe(() => LensManager.setActiveLens(id));
    const active = safe(() => LensManager.getActiveLens && LensManager.getActiveLens(), "?");
    record("activate " + label + " lens", active === id, { active, wanted: id });
    if (active !== id) safe(() => LensManager.setActiveLens("fxs-default-lens"));
  }
  const panel = safe(() => document.querySelector("lens-panel"), null);
  const emigButtons = panel ? safe(() => Array.from(panel.querySelectorAll("*")).filter((e) => {
    const s = String(safe(() => (e.getAttribute("id") || "") + " " + (e.getAttribute("class") || ""), ""));
    return /emig/i.test(s);
  }).length, 0) : 0;
  record("lens panel decorated", !!panel && emigButtons > 0, { panelFound: !!panel, emigElements: emigButtons });
}
...
function run() {
  emit("modtest104 start");
  safe(lensChecks);
  emit("SHOT lens_prosperity");
  safe(() => bannerChecks(() => {
    const pass = results.filter((r) => r.ok).length;
    emit("SUMMARY modtest104 " + pass + "/" + results.length + " checks OK");
    emit("FAILED: " + J(results.filter((r) => !r.ok).map((r) => r.name)));
    emit("SHOT banner_city");
    setTimeout(() => emit("DONE modtest104 finished"), 18000);
  }));
}

emit("modtest104 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") {
    setTimeout(() => { try { run(); } catch (e) { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest104 finished"); } }, 12000);
    return;
  }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest104 finished"); }
}
