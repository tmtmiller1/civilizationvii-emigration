// eep-modtest113.js - What combat evidence does the engine actually hand us, and does it reach into fog?
//
...
/**
 * Q6: is the per-city combat ledger actually filling from the live event stream? Reports every city of every
 * civ that recorded anything, so a distant AI-vs-AI siege shows up alongside the player's own fighting.
 */
function reportEvidence() {
  let scanned = 0, withEvidence = 0, foreign = 0;
  for (const p of safe(() => Players.getAlive(), []) || []) {
    ...
    for (const c of cities) {
      ...
      const ev = safe(() => globalThis.EmigrationCombat?.evidenceFor?.(key), null);
      if (!ev || (ev.dmg <= 0 && ev.battles <= 0 && ev.kills <= 0)) continue;
      withEvidence++;
      if (pid !== local) foreign++;
      emit("EVIDENCE '" + safe(() => Locale.compose(c.name), "?") + "' owner=" + pid
        + " dmg=" + Number(ev.dmg).toFixed(3) + " battles=" + ev.battles + " kills=" + ev.kills
        + " attackers=" + J(ev.attackers));
    }
  }
  emit("STATS " + J(safe(() => globalThis.EmigrationCombat?.stats?.(), "NO-GLOBAL")));
  emit("LEDGER citiesScanned=" + scanned + " withEvidence=" + withEvidence + " foreign=" + foreign);
}
...
function finish() {
  ...
  reportEvidence();
  emit("TALLY " + J(counts));
  setTimeout(() => emit("DONE modtest113 finished"), 4000);
}

function run() {
  local = safe(() => GameContext.localPlayerID, -1);
  emit("modtest113 run local=" + local);
  for (const ev of ["Combat", "UnitKilledInCombat", "DistrictDamageChanged"]) watch(ev);
  reportEvidence();
...
engine.on("PlayerTurnActivated", (d) => {
  ...
  setTimeout(() => {
    ...
    if (n % 3 === 0) reportEvidence();
    if (n >= TURNS) { finish(); return; }
    setTimeout(endTurn, 2000);
  }, 5000);
});

emit("modtest113 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") {
    setTimeout(() => { try { run(); } catch (e) { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest113 finished"); } }, 12000);
    return;
  }
  beginTries++;
  if (st === "WaitingToStart" || st === "WaitingForUIReady" || beginTries % 5 === 0) safe(() => UI.notifyUIReady());
  if (beginTries < 90) setTimeout(beginPoll, 2000); else { emit("LOAD gave up"); emit("DONE modtest113 finished"); }
}
