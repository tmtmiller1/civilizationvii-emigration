// eep-modtest119.js - Load check for the "Refugees from minor-power raids" Options slider.
//
...
const KEYS = ["LOC_OPTIONS_EMIG_MINORRAID", "LOC_OPTIONS_EMIG_MINORRAID_D", "LOC_EMIG_T_MINORVIOL", "LOC_EMIG_T_MINORFLOOR",
...
function run() {
  ...
  setTimeout(() => emit("DONE modtest119 finished"), 3000);
}
let tries = 0;
...
function poll() {
  if (stateName() === "GameStarted") { setTimeout(() => { try { run(); } catch (e) { emit("run threw " + e); emit("DONE modtest119 finished"); } }, 12000); return; }
  tries++;
  if (tries % 5 === 0 || tries < 3) safe(() => UI.notifyUIReady());
  if (tries < 90) setTimeout(poll, 2000); else { emit("LOAD gave up"); emit("DONE modtest119 finished"); }
}
emit("modtest119 attached");
