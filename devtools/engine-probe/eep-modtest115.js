// eep-modtest115.js - Are globals shared across mod script contexts at all?
//
  ...
  if (typeof g.EmigrationCombat === "object" && g.EmigrationCombat) {
    ...
    for (const p of safe(() => Players.getAlive(), []) || []) {
      ...
      for (const c of safe(() => Players.get(pid)?.Cities?.getCities?.(), []) || []) {
        ...
        const ev = safe(() => g.EmigrationCombat.evidenceFor(key), null);
        if (!ev || (ev.dmg <= 0 && ev.battles <= 0 && ev.kills <= 0)) continue;
        hits++;
        emit(tag + "   EVIDENCE '" + safe(() => Locale.compose(c.name), "?") + "' owner=" + pid
          + " dmg=" + Number(ev.dmg).toFixed(3) + " battles=" + ev.battles + " kills=" + ev.kills
          + " attackers=" + J(ev.attackers));
      }
    }
    emit(tag + "   citiesWithEvidence=" + hits);
  }
}
...
engine.on("PlayerTurnActivated", () => {
  ...
  setTimeout(() => {
    report("T" + n);
    if (n >= 8) { done = true; emit("DONE modtest115 finished"); }
  }, 4000);
});

emit("modtest115 attached");
let tries = 0;
...
function poll() {
  if (stateName() === "GameStarted") {
    setTimeout(() => {
      ...
      setTimeout(() => report("LATE"), 30000);
    }, 12000);
    return;
  }
  tries++;
  if (tries % 5 === 0 || tries < 3) safe(() => UI.notifyUIReady());
  if (tries < 90) setTimeout(poll, 2000); else { emit("LOAD gave up"); emit("DONE modtest115 finished"); }
}
