// eep-modtest62.js - mod test 61 with the screenshots fixed: a real pre-write shot, a closer camera, and any
...
// eep-modtest62.js - mod test 60 plus SCREENSHOTS (runner SHOT lines, captured by game window id): London and
...
async function closeScreens() {
  for (let i = 0; i < 6; i++) {
    const open = Array.from(document.querySelectorAll("fxs-close-button")).filter((b) => safe(() => b.getBoundingClientRect().width > 0, false));
    if (!open.length) return;
    safe(() => open[open.length - 1].dispatchEvent(new CustomEvent("action-activate", { bubbles: true })));
    emit("CLOSED a screen through its close button (" + open.length + " visible)");
    await later(2000);
  }
}
...
async function run() {
  ...
  emit("DONE modtest62 finished");
}

emit("modtest62 attached");
let beginTries = 0;
function loadStateName() { return safe(() => { const s = UI.getGameLoadingState(); for (const k of Object.keys(UIGameLoadingState)) if (UIGameLoadingState[k] === s) return k; return String(s); }, "?"); }
function beginPoll() {
  const st = loadStateName();
  if (st === "GameStarted") { setTimeout(() => { run().catch((e) => { emit("run threw " + e + " " + (e && e.stack)); emit("DONE modtest62 finished"); }); }, 10000); return; }
  beginTries++;
