...
const GUIDE = [
  ...
  },
  {
    ...
    faq: [
      { q: "What is the Ethnic Composition lens showing?", a: "Each settlement painted by where its people came from. Rather than one flat colour per city, every owned tile is shaded by an origin and by how densely it's populated: the urban core reads vivid in the dominant origin's colour, worked rural tiles fainter, and minorities cluster on the fringe, with each origin's share of the population preserved across the tiles. A city founded by one civilization, half-emptied by war, then captured and regrown shows the captor's colour strengthening over time, the demographic shift, on the map. Press Shift+E to toggle it." },
      { q: "What is the Migration Chronicle?", a: "A written history of the world's great migrations, in its own tab on the Migration screen. As large waves move, it records the moments worth keeping, a city emptied by war or disaster, a diaspora taking root far from home, a people returning once their homeland recovered, each as a short line of prose. The per-event detail still lives in Notifications; the Chronicle is the story those events tell." },
      ...
      { q: "How do I turn the new identity systems on or off?", a: "All three are on by default and each has its own switch under Options ▸ Mods ▸ Emigration: \"ethnic integration\" (newcomers drifting toward the host identity), \"return migration\" (diasporas going home), and \"refugee decisions\" (the occasional pop-up). The Ethnic Composition lens and the Chronicle tab are always available." }
    ]
  }
];
