...
const GUIDE = [
  ...
  },
  {
    ...
    rows: [
      ...
      { q: "Absorbing migrants is free", yes: false, note: "No. Receiving migrants adds a temporary, decaying integration cost in happiness and gold." },
      { q: "War alone can empty a city to zero", yes: false, note: "No. War DISPLACEMENT (people fleeing) is capped (siegeLossCapPct) and never takes a city below its rural floor. Crisis DEATHS are a separate channel and are not floored that way: a prolonged crisis (war, siege, famine) keeps killing some who can't escape, so a long enough catastrophe can wear a city's rural population all the way down. The dying starts gentle and builds the longer the crisis lasts, and it only ever removes rural population, so the urban core and the settlement itself survive until an actual capture." },
  ...
  },
  {
    ...
    rows: [
      ...
      { q: "War or unrest keeps a community distinct", yes: true, note: "Integration stalls while the host is at war with a diaspora's homeland, and slows in a city in unrest, so a contested or resentful city holds a distinct, unintegrated community that keeps its colour on the lens." },
  ...
  },
  {
    ...
    faq: [
      ...
      { q: "Do people die, or just move away?", a: "Both, depending on the cause. Ordinary (prosperity / unhappiness) migration NEVER kills; people simply leave for better cities. But a LETHAL crisis (war, siege, disaster, or famine) also kills some who can't escape in time: those deaths leave the world entirely (shown in the Losses/deaths tally, separate from refugees), even while the rest flee. So a city under crisis loses people two ways at once: most flee, a few die. The death toll builds up gradually rather than striking all at once, growing the longer the crisis drags on and easing if it lifts, so a brief scare is recoverable while a grinding siege or famine is not. Separately, a refugee can also perish IN TRANSIT if the city they were fleeing to is captured or razed before they arrive, or stays too full to take them in." }
    ]
  },
  {
    ...
    faq: [
      ...
      { q: "How far can a war shrink a city?", a: "War DISPLACEMENT (people fleeing) is capped at 60% (siegeLossCapPct) of the city's population when the siege began; the remnant \"digs in\" and can't be pushed out further. A sustained siege also kills some who can't escape. Those deaths are not bound by the displacement cap or the rural floor, so a long enough siege can wear the city's rural population all the way down, though its urban core and the settlement still stand until an actual capture. The dying builds up gradually rather than all at once, and eases if the siege lifts. Only an actual capture takes the city." },
  ...
  },
  {
    ...
    faq: [
      { q: "What happens to a migrant while they're traveling between cities?", a: "Migration isn't instant; it has transit lag. The migrant leaves the source the turn they depart: its rural population point (and that worker's tile yields) is removed right away. They then spend the transit time belonging to NO city, working no tiles, producing nothing, but also costing no upkeep, gold, or happiness. They're added to the destination, which only then gains the yields and pays its one-time integration cost, when they ARRIVE. Transit is 1-4 turns, scaled by distance (~5 hexes per turn, capped at 4); refugees take at least 1 (they camp). Arrival isn't guaranteed: a city can only absorb so many newcomers per turn (the per-city intake cap), so if the destination is full that turn the migrant waits in transit and tries again next turn. And if it was captured or razed while they traveled (or they still can't find room after several turns of waiting) the refugees perish in transit (counted as a loss, not an arrival)." },
      ...
      { q: "Is there a limit on how many people one city gains or loses per turn?", a: "Yes, both ways. A settlement can shed at most a few migration points per turn (so even a besieged city bleeds people steadily instead of emptying in a single catastrophic turn) and it can take in at most a few, so one boomtown can't swallow dozens of newcomers at once. Arrivals over the intake cap simply wait in transit and land on a later turn. Both caps scale with the Emigration intensity preset (Low / Medium / High) and can be fine-tuned individually in Advanced settings. Deaths from war, famine and disaster are a separate channel and aren't counted against either cap." }
    ]
  },
  {
