...
// dialog — DialogBoxManager.createDialog_MultiOption, the same system the base game uses for its goody-hut
// / diplomacy / system decisions. Input is owned by the engine's dialog layer, so the choice buttons and
// the ✕ / Escape ALWAYS receive clicks.
//
// WHY THIS REPLACED A HAND-ROLLED PANEL: an earlier build rendered a ContextManager Panel styled to look
// like this pop-up. Even mouse-guard-backed, its buttons stayed dead on some setups — the "the choices are
// unclickable and there's no way to X out" bug players kept reporting. Using the real engine dialog removes
// that entire class of failure: we no longer own the input path, the engine does.
//
...
//   choices[] → one option button each (label + the consequence cue as its hover tooltip)
//   dismissId → the option wired to Escape / the ✕ / cancel (default "away")
...
/**
 * Map the view's choices to native dialog options. Each option resolves the decision once with its id;
 * the option whose id is the dismiss option is also wired to Escape / cancel / the ✕. A safety-net cancel
 * option is appended only if no choice carries the dismiss id (real callers always include it, so this
 * never adds a visible button in practice — it just guarantees a dismiss path exists).
 ...
 * @returns {{actions:string[], label:string, tooltip?:string, callback:()=>void}[]} The dialog options.
 */
...
/**
 * Show the decision pop-up in the game's own native dialog. Resolves with the chosen option id (Escape /
 * the ✕ / cancel resolve as `view.dismissId`, default "away"). Calls `onChoice(id)` exactly once. A silent
 * no-op where the core dialog module is unavailable (shell / node tests).
