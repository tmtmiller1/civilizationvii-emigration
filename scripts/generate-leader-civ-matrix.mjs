...
// Read the INSTALLED game, so the roster reflects every civilization and leader the engine actually
// loads, DLC included. A pinned snapshot goes stale the moment a DLC ships and silently freezes the
// roster: that is how Elizabeth, Washington, Yi Sun-sin, Babylon, England, Gaul, Goryeo and Joseon
// stayed out of the matrix. Override with CIV7_ROOT for a different install or an archived build.
const DEFAULT_INSTALL = path.join(
  process.env.HOME || "",
  "Library/Application Support/Steam/steamapps/common",
  "Sid Meier's Civilization VII/CivilizationVII.app/Contents"
);
const FALLBACK_SNAPSHOT = path.resolve(EMIGRATION_ROOT, "..", "..", "civilization_vii_1.4.1_game_files");

/** @param {string} root @returns {boolean} Whether root holds a readable Resources tree. */
function hasResources(root) {
  try {
    return fs.statSync(path.join(root, "Resources")).isDirectory();
  } catch {
    return false;
  }
}

const CIV7_ROOT = [process.env.CIV7_ROOT, DEFAULT_INSTALL, FALLBACK_SNAPSHOT]
  .filter((root) => typeof root === "string" && root.length > 0)
  .find((root) => hasResources(/** @type {string} */ (root)));
if (!CIV7_ROOT) {
  console.error(
    "No Civilization VII Resources tree found. Set CIV7_ROOT to an install or an archived build."
  );
  process.exit(1);
}
...
/**
 * The leader type as the ENGINE reports it. An alternate persona keeps its own `_ALT` type: the
 * runtime looks tuning up by exact type, and the two personas are different leaders for migration
 * (Ashoka renounces the world in one and conquers it in the other), so folding them together both
 * missed the alternate at runtime and would have given it the wrong profile.
 * @param {string} leader The raw LeaderType. @returns {string} The same type.
 */
...
// A stable label, not the absolute path: the path differs per machine and would churn this generated
// file (and overrun the line limit) on every regeneration.
const SOURCE_LABEL = "the installed Civilization VII Resources tree (override with CIV7_ROOT)";
...
const md = [
  ...
  "",
  `Generated from game XML in ${SOURCE_LABEL}.`,
  "",
  "## Coverage Summary",
  "",
